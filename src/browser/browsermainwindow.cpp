...
void BrowserMainWindow::setupMenu()
{
    ...
    //helpMenu->addAction(tr("About &KitBrowser"), this, SLOT(slotAboutApplication()));
}
...
{
    if (title.isEmpty()) {
        setWindowTitle(tr("KitBrowser"));
    } else {
#if defined(Q_WS_MAC)
        setWindowTitle(title);
#else
        setWindowTitle(tr("%1 - KitBrowser", "Page title and Browser name").arg(title));
#endif
    }
}
...
void BrowserMainWindow::slotHome()
{
    ...
    QString home = settings.value(QLatin1String("home"), QLatin1String("http://www.qtchina.net/")).toString();
    loadPage(home);
}
